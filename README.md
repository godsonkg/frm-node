...
当前版本：`0.2.0`。已在 6 台真实 VPS 上完成接管、监控与订阅导出的实机验证；仍是个人自用工具，未做多发行版与多架构的完整验证。安全边界与已知取舍见 [SECURITY.md](SECURITY.md)。
...
```bash
FRM_REF=v0.2.0 bash <(curl -fsSL https://raw.githubusercontent.com/godsonkg/frm-node/v0.2.0/bootstrap.sh)
```

安装器会打印安装包的 SHA-256。记下首次可信安装的值，之后可加 `FRM_SHA256` 强制校验，内容不符即中止：

```bash
FRM_REF=v0.2.0 FRM_SHA256=<上次记下的校验值> bash <(curl -fsSL .../bootstrap.sh)
```
...
frm cert pin all         # 给所有支持的实例补录指纹
frm cert pin <实例>       # 单个实例
frm cert pin <实例> --cert <证书路径>   # 自动定位失败时手工指定
frm cert unpin <实例>     # 移除钉扎
```

指纹来源：TCP 协议直接探测本机端口（拿到的正是客户端实际看到的证书，与来源脚本无关）；QUIC/UDP 协议回退到定位证书文件。补录只写实例凭据文件，**不改动节点配置、证书或服务状态**。

支持矩阵与拒绝理由：

| 协议 | 是否钉扎 | 原因 |
|---|---|---|
| Hysteria2、Trojan | ✅ | 使用长期固定自签证书 |
| AnyTLS | ❌ | `anytls-server` 无法指定证书，每次启动自签一张 2 小时有效期的临时证书；钉扎会导致重启后断连 |
| Reality | ❌ | 借用真实站点证书，客户端凭 `public-key` 验证，无需指纹 |
| Snell | ❌ | 不使用 TLS 证书 |

**内置安全阀**：证书有效期短于 30 天一律拒绝钉扎并说明原因，避免把临时证书钉死导致节点断连。

钉扎结果目前输出到 Mihomo/Clash 的 `fingerprint` 字段与 Hysteria2 的 `pinSHA256` URI 参数。Surge、Loon 的对应参数尚未确认，暂不输出，以免生成不被接受的配置。补录后需重新生成订阅，并确认客户端能正常解析与连接。

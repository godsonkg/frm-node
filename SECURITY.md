...
2. **AnyTLS 无法提供服务端认证（已实测确认）**：`anytls-server` 只接受 `-l`、`-p`、`-padding-scheme` 三个参数，**没有任何指定证书的选项**；它在启动时自行生成一张有效期仅 2 小时、subject 为空的自签证书，且重启后即更换。实测三台机器的证书均已过期、指纹互不相同。

   因此 AnyTLS 的 `skip-cert-verify` 是它能连通的**必要条件**，指纹钉扎在此协议上不可行（钉扎后每次重启都会断连）。**AnyTLS 的安全性完全依赖密码本身，不提供服务端身份认证**：能实施在途劫持的攻击者可以冒充服务端。若你处在不可信网络（公共 Wi-Fi、可疑网络环境），请优先使用 **Reality**（有 public-key 验证）而非 AnyTLS。

3. **Trojan / Hysteria2 的 `skip-cert-verify`**：这些节点以纯 IP 提供服务、没有域名，无法签发受信证书，但它们使用的是**长期固定的自签证书**（实测 Trojan 证书有效期至 2126 年），因此可以用 SHA-256 指纹钉扎替代 CA 验证。frm-node 原生创建的 Hysteria2 会自动记录并导出指纹；从旧脚本接管的实例凭据中没有指纹字段，需要补录后才享有该保护。

4. **Xray / AnyTLS / Hysteria / ShadowTLS 跟随上游 latest**：未锁定版本与校验值，以换取安全更新的及时性和可维护性。核心更新前自动备份，启动失败自动回滚二进制。Snell 已钉扎校验值，见上文供应链一节。

5. **以 root 运行**：写 systemd unit、安装二进制、绑定端口所必需，无法避免。

6. **接管实例受保护**：来自旧脚本的 `adopted` / `taken-over` 实例禁止普通 `update` 与 `uninstall`，避免误伤共享核心。
